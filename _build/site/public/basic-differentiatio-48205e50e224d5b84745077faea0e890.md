...
A smooth function $z(x)$ can be approximated around $x=a$ by
...
A better approximation would be a linear function of the form $h(x) = f(a) + b(x-a)$, where $b$ is some slope. But what would be a good value of $b$? We saw above that the differential is an equation for the tangent line (or slope) at the point $x = a$. So, we could argue that the best linear approximation to the function around this point would be
...
where $f'(a)$ is the derivative of the function evaluated at $x=a$.

But why stop here? We could improve on this. A better approximation could allow for some curvature. The general form would then be, say,  $f(x) = f(a) + f '(a) .(x-a) + c .(x-a)^2$. Again, we ask, "What would be the best value for c?" The rate of change of the slope of the quadratic approximation should be equal to the rate of change of change of the function at the $a$. And since the second derivative of $f(x)$ is $2c$, then for $f''(x)$ to equal $z''(x)$ at $x = a$, we need $c = 1/2 f''(a)$. Hence the quadratic approximation to the function aound $x = a$ is:
...
$z(x)$ is the original function being approximated (the solid curve).$g(x)$ represents the first-order Taylor polynomial-, i.e. a constant function. $h(x)$ is a straight line that has the same value and slope as $z(x)$ at $x=a$ i.e. it is tangent to $z(x)$ at $x=a$. The formula for $f(x)$ is $f(x) = f(a) + f^{\prime }(a)(x-a)\)\(h(x)$ represents a higher-order Taylor polynomial, i.e. the second-order (quadratic) polynomial-the dashed curve. This approximation matches better the function's value, slope, and concavity (curvature) at $x=a$, which makes it a better approximation of $z(x)$ near $x=a$ than the linear approximation $h(x)$. The graph demonstrates that as more terms are included in the Taylor polynomial, the approximation of the original function becomes more accurate over a larger range around the center point $x=a$. 
