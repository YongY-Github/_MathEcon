...
Young’s Theorem shows that a multivariate function that is fully differentiable with respect to all its $n$ arguments has, at most, $n$ distinct second partial derivatives, and $(n^2 - n)/2$ distinct cross partial derivatives.

Continuing with the Cobb–Douglas production function in previous section above,
...
which is positive since $K$ and $L$ are positive. The economic interpretation is that the marginal product of either input rises as more of the other input is used.
