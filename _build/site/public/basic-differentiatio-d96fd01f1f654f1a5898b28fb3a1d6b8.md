...
This represents the change in $y$ along the tangent line at $x_0$. Graphically, this is shown in {numref}`fig-diff` 
...
**(a)** We make the main assumption that $y$ is a function of $x$, i.e. $y=f(x)$.  
...
**(b)** Solving the resulting equation for $y'$ gives
...
1. differentiating $f$ with respect to $x$ to obtain $f_x$,  
2. differentiating $f$ with respect to $y$ to obtain $f_y$,  
3. taking the ratio $-\dfrac{f_x}{f_y}$.
...
::::{admonition} Intuition: elasticity of substitution
:class: note
