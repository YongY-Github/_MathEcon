...
[^1]: A function need not be defined at $a$ in order to have a limit as $x \to c$.
...
The first is not continuous because $f(c)$ is not defined.
The second is not continuous because $f$ does not converge to a limit as $x \to c$.
For example, if
:::{math}
:enumerated: false
f(x) =
