...
The general exponential function rule$$\frac{\partial}{\partial x} e^{g(x)} = e^{g(x)} g'(x)$$For example:$$\frac{\partial}{\partial x} e^{ax} = \frac{\partial}{\partial (ax)} e^{ax} \frac{\partial}{\partial x} (ax) = e^{ax} a = ae^{ax}$$If we are using a base other than $e$:
